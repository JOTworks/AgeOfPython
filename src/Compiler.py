...
def check_terminal_nodes(tree, node_type):
    for node in ast.walk(tree):
        if isinstance(node, ast.AST):
            children = [getattr(node, field) for field in node._fields if hasattr(node, field)]
            if all(not isinstance(child, ast.AST) for child in children):
                if not (isinstance(node, node_type)
                        or isinstance(node, (ast.BoolOp, ast.And, ast.Or, ast.Not, ast.Load))
                        ):
                    raise TypeError(f'Terminal node {ast.dump(node)} is not {node_type}')
...
class Command(ast.Call):
    def __init__(self, name: AOE2FUNC, args: list, node):
        ...
        for arg in args:
            if not (isinstance(arg, JumpType)
...
class Compiler:
    def __init__(self):
        self.command_names = [name for name, obj in inspect.getmembers(aoe2scriptFunctions, inspect.isfunction)]
