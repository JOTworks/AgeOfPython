...
class Memory:  # TODO: openMemory away. we can just use used Memory for everything! None instead of ""
    ...
    def getMemLoc(self, varName):
        ...
        if not self.isUsed(varNameList[0]):
            raise Exception(
                "Can't get memLoc that dosnt exist: "
                + varName
                + "\n"
        ...
        if len(varNameList) == 1:
            return memStartLoc
        if (len(varNameList) == 2) and (
            self.__usedMemory[memStartLoc + 1] == Structure.POINT
        ):
        ...
        if (len(varNameList) == 2) and (
            self.__usedMemory[memStartLoc + 1] == Structure.STATE
        ):
    ...
    def malloc(self, varName, s_type):
        if not isinstance(varName, str):
            raise Exception(
                "what are you doing trying to store a none string into memory???"
            )
    ...
    def get_current_func_name(self):
        raise Exception(
            "DEPRICATED: only works if every function allocats something to Mem"
        )
        func_name = "Main"
        funcLen = 2
    ...
    def free(self, varName):
        ...
        if self.__usedMemory[index + 1] == Structure.POINT:
            ...
            raise Exception(
                f"structure type {self.__usedMemory[index + 1]} is not implemented in Free()"
            )
