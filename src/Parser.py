...
class Aoe2Tree:
    def __init__(self, main_tree = ast.Module(body=[]), func_tree = ast.Module(body=[]), const_tree = ast.Module(body=[])):
        ...
        for tree_filename, tree in asts.items():
            for node in tree.body:
                if isinstance(node, ast.FunctionDef):
                    ...
                    raise Exception(f"Only FunctionDefs and Assigns allowed in imports, not {type(node)}")
                ...
                for node in tree.body:
                    if isinstance(node, ast.FunctionDef) and (node.name in alias_names  or import_all):
                      ...
                      if isinstance(node.targets[0], ast.Name) and (node.targets[0].id in alias_names or import_all):
                        ...
                        if not ('aoe2scriptEnums' in node.module or 'aoe2scriptFunctions' in node.module or 'scraper' in node.module):
